<script lang="ts">
  ...
  let option: echarts.ComposeOption<
    TooltipComponentOption | GridComponentOption | CustomSeriesOption
  >

  $: date = dayjs(durations.start).format(DateFormat.Shortish)
  $: option = createDurationsChartOption(durations, itemType) as echarts.ComposeOption<
    TooltipComponentOption | GridComponentOption | CustomSeriesOption
  >
...
<ChartContainer>
  ...
  <DailyChartControls {durations} on:update={onUpdate} {itemType} />
